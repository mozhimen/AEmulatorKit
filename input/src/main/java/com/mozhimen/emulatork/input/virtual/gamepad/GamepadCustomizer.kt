...
class GamepadCustomizer {
    ...
    fun displayCustomizationPopup(
        ...
        return getEvents(activity, layoutInflater, view, gamepadSetting, insets)
    ...
    private fun getEvents(
        ...
        var (scale, rotation, marginX, marginY) = gamepadSetting
        ...
        editControlsWindow?.contentView?.findViewById<Button>(com.mozhimen.emulatork.input.R.id.edit_control_reset)
            ?.setOnClickListener {
                scale = GamepadSetting.DEFAULT_SCALE
                rotation = GamepadSetting.DEFAULT_ROTATION
                marginX = GamepadSetting.DEFAULT_MARGIN_X
                marginY = GamepadSetting.DEFAULT_MARGIN_Y
        ...
        touchDetector = MultiTouchGestureDetector(
            ...
            object : MultiTouchGestureDetector.SimpleOnMultiTouchGestureListener() {
                val moveScale: Float = GamepadSetting.MAX_MARGINS.dp2px()
                ...
                override fun onRotate(detector: MultiTouchGestureDetector) {
                    val currentRotation = rotation * GamepadSetting.MAX_ROTATION
                    val nextRotation = currentRotation - invertXAxis * detector.rotation
                    rotation = MathUtils.clamp(
                        nextRotation / GamepadSetting.MAX_ROTATION,
                        0f,
                        1f
                    )
                    events.value = Event.Rotation(rotation)
