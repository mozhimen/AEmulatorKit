...
/**
 * @ClassName TouchControllerCustomizer
 * @Description TODO
 * @Author Mozhimen & Kolin Zhao
 * @Date 2024/5/13
 * @Version 1.0
 */
class ControllerTouchCustomizer {
    ...
    private fun getEvents(
        ...
        val contentView = layoutInflater.inflate(com.mozhimen.emulatork.input.R.layout.layout_edit_touch_controls, null)
        ...
        editControlsWindow?.contentView?.findViewById<Button>(com.mozhimen.emulatork.input.R.id.edit_control_reset)
            ?.setOnClickListener {
                scale = ControllerTouchSettingsManager.DEFAULT_SCALE
                rotation = ControllerTouchSettingsManager.DEFAULT_ROTATION
                marginX = ControllerTouchSettingsManager.DEFAULT_MARGIN_X
                marginY = ControllerTouchSettingsManager.DEFAULT_MARGIN_Y

                events.value = Event.Margins(marginX, marginY)
                events.value = Event.Rotation(rotation)
                events.value = Event.Scale(scale)
            }
        editControlsWindow?.contentView?.findViewById<Button>(com.mozhimen.emulatork.input.R.id.edit_control_done)
            ?.setOnClickListener {
        ...
        touchDetector = MultiTouchGestureDetector(
            ...
            object : MultiTouchGestureDetector.SimpleOnMultiTouchGestureListener() {
                val moveScale: Float = ControllerTouchSettingsManager.MAX_MARGINS.dp2px()
                ...
                override fun onRotate(detector: MultiTouchGestureDetector) {
                    val currentRotation = rotation * ControllerTouchSettingsManager.MAX_ROTATION
                    val nextRotation = currentRotation - invertXAxis * detector.rotation
                    rotation = MathUtils.clamp(
                        nextRotation / ControllerTouchSettingsManager.MAX_ROTATION,
                        0f,
                        1f
                    )
                    events.value = Event.Rotation(rotation)
