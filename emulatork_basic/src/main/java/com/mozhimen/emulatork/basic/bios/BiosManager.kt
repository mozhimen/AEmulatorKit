 ...
 * @Version 1.0
 */
class BiosManager(private val storageDirectoriesManager: StorageDirectoriesManager) {
    ...
    fun getMissingBiosFiles(coreConfig: GameSystemCoreConfig, game: Game): List<String> {
        ...
        return (coreConfig.requiredBIOSFiles + requiredRegionalFiles)
            .filter { !File(storageDirectoriesManager.getSystemDirectory(), it).exists() }
    }

    fun deleteBiosBefore(timestampMs: Long) {
        Timber.i("Pruning old bios files")
        SUPPORTED_BIOS
            .map { File(storageDirectoriesManager.getSystemDirectory(), it.libretroFileName) }
    ...
    fun getBiosInfo(): BiosInfo {
        val bios = SUPPORTED_BIOS.groupBy {
            File(storageDirectoriesManager.getSystemDirectory(), it.libretroFileName).exists()
        }.withDefault { listOf() }
    ...
    fun tryAddBiosAfter(
        ...
        val biosFile = File(storageDirectoriesManager.getSystemDirectory(), bios.libretroFileName)
